...
@implementation ContentURLGetter

@synthesize currentVideo;

static ContentURLGetter *singletonContentURLGetter = nil;

+ (ContentURLGetter *)singleton
{
    if (singletonContentURLGetter == nil) {
        singletonContentURLGetter = [[super allocWithZone:NULL] init];
    }
    return singletonContentURLGetter;
}
...
- (void)getYouTubeURL
{
    ...
    @synchronized(self) {
        if (self.currentVideo == nil || self.currentVideo.contentURL) {
            return;
        }
        _lastGetBegan = CACurrentMediaTime();
